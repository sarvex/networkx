...
def test_all_simple_paths_directed():
    ...
    assert_equal([list(p) for p in paths], [[1,2,3]])

def test_all_simple_paths_empty():
    G = nx.path_graph(4)
    paths = nx.all_simple_paths(G,0,3,cutoff=2)
    assert_equal([list(p) for p in paths], [])
